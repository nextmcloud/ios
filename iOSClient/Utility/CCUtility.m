...
{
    NCPermissions *permissions = [[NCPermissions alloc] init];
    NSInteger permissionsValue = permissions.permissionReadShare;

    if (canEdit && !isFolder) {
        permissionsValue = permissionsValue + permissions.permissionUpdateShare;
    }
    if (canCreate & isFolder) {
        permissionsValue = permissionsValue + permissions.permissionCreateShare;
    }
    if (canChange && isFolder) {
        permissionsValue = permissionsValue + permissions.permissionUpdateShare;
    }
    if (canDelete & isFolder) {
        permissionsValue = permissionsValue + permissions.permissionDeleteShare;
    }
    if (canShare) {
        permissionsValue = permissionsValue + permissions.permissionShareShare;
    }
...
+ (BOOL) isPermissionToCanCreate:(NSInteger) permissionValue {
    BOOL canCreate = ((permissionValue & [[NCPermissions alloc] init].permissionCreateShare) > 0);
    return canCreate;
}

+ (BOOL) isPermissionToCanChange:(NSInteger) permissionValue {
    BOOL canChange = ((permissionValue & [[NCPermissions alloc] init].permissionUpdateShare) > 0);
    return canChange;
}

+ (BOOL) isPermissionToCanDelete:(NSInteger) permissionValue {
    BOOL canDelete = ((permissionValue & [[NCPermissions alloc] init].permissionDeleteShare) > 0);
    return canDelete;
}

+ (BOOL) isPermissionToCanShare:(NSInteger) permissionValue {
    BOOL canShare = ((permissionValue & [[NCPermissions alloc] init].permissionShareShare) > 0);
    return canShare;
}
...
+ (BOOL) isPermissionToRead:(NSInteger) permissionValue {
    BOOL canRead = ((permissionValue & [[NCPermissions alloc] init].permissionReadShare) > 0);
    return canRead;
}
