...
class NCAppConfigView: UIViewController {
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        guard let serverUrl = self.serverUrl else {
            NCContentPresenter.shared.messageNotification("_error_", error: NKError(errorCode: NCGlobal.shared.errorInternalError, errorDescription:"User Default, serverUrl not found"), delay: NCGlobal.shared.dismissAfterSecond, type: NCContentPresenter.messageType.error)
            return
        }
        guard let username = self.username else {
            NCContentPresenter.shared.messageNotification("_error_", error: NKError(errorCode: NCGlobal.shared.errorInternalError, errorDescription: "User Default, username not found"), delay: NCGlobal.shared.dismissAfterSecond, type: NCContentPresenter.messageType.error)
            return
        }
        guard let password = self.password else {
            NCContentPresenter.shared.messageNotification("_error_", error: NKError(errorCode: NCGlobal.shared.errorInternalError, errorDescription: "User Default, password not found"), delay: NCGlobal.shared.dismissAfterSecond, type: NCContentPresenter.messageType.error)
            return
        }
            ...
            DispatchQueue.main.async {
                if error == .success && token != nil {
                    ...
                    guard let tableAccount = NCManageDatabase.shared.setAccountActive(account) else {
                        NCContentPresenter.shared.messageNotification("_error_", error: NKError(errorCode: NCGlobal.shared.errorInternalError, errorDescription: "setAccountActive error"), delay: NCGlobal.shared.dismissAfterSecond, type: NCContentPresenter.messageType.error)
